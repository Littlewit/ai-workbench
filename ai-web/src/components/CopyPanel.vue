...
<template>
  <div class="pane" style="padding:20px">
    <h3>AI 文案生成</h3>
    <textarea v-model="prompt" rows="4" placeholder="输入需求：例如写一篇朋友圈、产品介绍、工作总结..." class="prompt-input" />
    <button @click="run" :disabled="loading" class="btn-primary">{{ loading ? '生成中...' : '开始生成' }}</button>
...
const run = async () => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/ai`, {
      ...
      body: JSON.stringify({
        messages: [{ role: 'user', content: '你是专业文案，只返回文案：' + prompt.value }]
      })
    });
